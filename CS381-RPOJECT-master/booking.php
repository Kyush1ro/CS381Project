...
if (isset($_GET['movie_id']) && isset($_GET['time_id'])) {
  $db = new db();
  $db->connect();
  ...
  if ($result) {
    ...
    $result[0]->Genre = str_replace(";", ",", $result[0]->Genre);
  ...
  $sql = "SELECT * FROM available_booking_time_tbl WHERE time_id = '$time_id'";
  $result = $db->get_records($sql);
  ...
  $sql = "SELECT * FROM available_booking_date_tbl WHERE date_id = '$date_id' and movie_id = '$movie_id'";
  $result = $db->get_records($sql);
...
<head>
  ...
  <title>Book Tickets - FOX cinema</title>
  <link rel="stylesheet" href="css/style.css" />
  <link rel="stylesheet" href="https://www.w3schools.com/w3css/5/w3.css">
  <link rel="stylesheet" href="css/w3.css" />
  <link rel="website icon" href="images/logo.png">
  <script>
    const movie_id = <?= json_encode($movie_id) ?>;
    const time_id = <?= json_encode($time_id) ?>;
  </script>
...
<body>
  ...
  <!-- Main Content -->
  <div class="booking-container w3-container">
    <div class="movie-details-b w3-round-large">
      <img src="images/movies/<?= $movie_id ?>.jpg" alt="Movie Poster" class="movie-poster-small" style="width: 120px; hight: 180px" />
      <div class="movie-info-details">
        <h1><?= $Title ?></h1>
        <div class="movie-metadata">
          <span> <?= $Genre ?> | <?= $Runtime ?> Min | <?= $Certificate ?></span>
        </div>
        <div class="screening-details">
          <p><?= $Date ?> | <?= $Time ?> | Cinema Hall 1</p>
        </div>
      </div>
    </div>
    ...
    <div class="booking-section" id="div1">
      <h2 class="w3-text-dark-grey">Select Your Seats</h2>
      <p>Click on available seats to select them. Selected seats will be highlighted in red.</p><br><br>
      ...
      <div class="seats-container">
        ...
        <div class="seat-row">
          <div class="row-identifier">A</div>
        ...
        <div class="seat-row">
          <div class="row-identifier">B</div>
        ...
        <div class="seat-row">
          <div class="row-identifier">C</div>
        ...
        <div class="seat-row">
          <div class="row-identifier">D</div>
        ...
        <div class="seat-row">
          <div class="row-identifier">E</div>
        ...
        <div class="seat-row">
          <div class="row-identifier">F</div>
    ...
    <div class="booking-section" id="div2">
      <h2 class="w3-text-dark-grey">Enter Person Details</h2>
      <p>Please provide details for ticket.</p>

      <div class="persons-details">

        <div class="person-form w3-round-large">
          <div class="person-header">
            <h3>Person 1 - Seat <div class="seat_num"></div>
            </h3>
          </>
          ...
          <div class="form-row">
            <div class="form-group">
              ...
              <input id='fn' type="text" class="form-control w3-input" placeholder="Enter first name">
            </div>
            <div class="form-group">
              <label class="form-label">Last Name</label>
              <input id='ln' type="text" class="form-control w3-input" placeholder="Enter last name">
            </div>
          </div>
          ...
          <div class="form-row">
            <div class="form-group">
              ...
              <input id='email' type="email" class="form-control w3-input" placeholder="Enter email">
            </div>
            <div class="form-group">
              <label class="form-label">Phone</label>
              <input id='phone' type="tel" class="form-control w3-input" placeholder="Enter phone number" maxlength="10">
            </div>
          </div>
        </div>
        ...
        <!-- <div class="person-form w3-round-large">
          <div class="person-header">
            <h3>Person 2 - Seat F6</h3>
          </div>
          
          <div class="form-row">
            <div class="form-group">
              <label class="form-label">First Name</label>
              <input type="text" class="form-control w3-input" placeholder="Enter first name">
            </div>
            <div class="form-group">
              <label class="form-label">Last Name</label>
              <input type="text" class="form-control w3-input" placeholder="Enter last name">
            </div>
          </div>
          ...
          <div class="form-row">
            <div class="form-group">
              <label class="form-label">Email</label>
              <input type="email" class="form-control w3-input" placeholder="Enter email">
            </div>
            <div class="form-group">
              <label class="form-label">Phone</label>
              <input type="tel" class="form-control w3-input" placeholder="Enter phone number">
            </div>
          </div>
        </div> -->
      </div>
    ...
    <div class="booking-section" id="div3">
      <h2 class="w3-text-dark-grey">Booking Summary</h2>

      <div class="booking-summary w3-round-large">
    ...
    <div class="action-buttons" id="div4">
      <button class="back-btn w3-button">Back</button>
      <button class="continue-btn w3-button" onclick='validate_payment(event)'>Continue to Payment</button>
  ...
  <script src="JS/handleseats.js"></script>
  <script src="JS/handleprogress.js"></script>
  <script>
    document.addEventListener("DOMContentLoaded", function() {
      time_id = <?= $time_id ?>;
      console.log(time_id);
      fetch("get.seats.php?time_id=" + time_id, {
        headers: {
          "Content-Type": "application/json"
        },
        method: "GET"
      }).then(res => res.json()).then(data => {
        occupiedSeats = data.result;
        if (occupiedSeats == "") {
          return;
        }
        occupiedSeats.forEach(seat => {
          document.querySelector("[data-seat=" + seat + "]").classList.add("occupied");
        })
      })
    })
  </script>
